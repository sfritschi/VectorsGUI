...
public class VectorFrame extends JComponent implements ActionListener {
    ...
    private void handleUndo() {
        ...
        if (lines.size() > 0) {
            Line2D removedLine = lines.get(lines.size() - 3);
            for (int i = 0; i < 3; i++)
                lines.remove(lines.size() - 1);
            Point2D savePoint;
            if (lines.size() > 0) {
                savePoint = lines.get(lines.size() - 3).getEnd();
            } else {
    ...
    public Vector2D handleVector(String x, String y, Point2D p) {
        try {
            ...
            if (p != null) {
                ...
                boolean isZeroVector = xComp == 0 && yComp == 0;
                
                if (xComp + p.getX() <= VectorCanvas.WIDTH &&
                    yComp + p.getY() <= VectorCanvas.HEIGHT
                    && isInBounds && !isZeroVector) {
